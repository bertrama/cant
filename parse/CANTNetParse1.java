...
//System.out.println(neuronNum+ " " + neurons[neuronNum].synapses[synapseNum].getTo().getId());
  ...
  //Initially only one stack element.  It should have some fast
  //binding synapses to verb and noun. 
	  ...
	  for (int synapseNum=0;synapseNum < neurons[neuronNum].getCurrentSynapses(); synapseNum++) 
		...
		if ((neuronNum / 300) == (toNeuron / 300)) 
	      ...
	      if (neurons[neuronNum].isInhibitory)
	        ...
	        neurons[neuronNum].synapses[synapseNum].setWeight(0.6);
		  }
		else	
	      { 
	      if (neurons[neuronNum].isInhibitory)
	        neurons[neuronNum].synapses[synapseNum].setWeight(-0.8);
	      else   
	        neurons[neuronNum].synapses[synapseNum].setWeight(0.01);
	      }
	    }  
  ...
  private void connectPushRuleToStack(CANTNetParse1 stackNet) {
    for (int ruleNeuron = 0; ruleNeuron < 300; ruleNeuron ++) 
      {
      for (int synapse = 0; synapse < 10; synapse++)
        {
        if (!neurons[ruleNeuron].isInhibitory())
          neurons[ruleNeuron].addConnection(stackNet.neurons[(int)(Math.random()*300)],0.4);
        }
      }
  }

  public void connectRulesToStack(CANTNetParse1 stackNet) {
    connectPushRuleToStack(stackNet);
  }

  public void connectStackToVerb(CANTNetParse1 verbNet) {
    for (int neuronNum=0; neuronNum<size();neuronNum++) 
	  {
	  if ((neuronNum % 300) > 150)
        for (int synapseNum=0;synapseNum < 50; synapseNum++) 
          ((CANTNeuronFastBind)neurons[neuronNum]).addConnection(
	        verbNet.neurons[(int)(Math.random()*1200)],0.1);
